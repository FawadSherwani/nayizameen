  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-primary-50 to-white">
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 px-4 pt-10 md:grid-cols-2 md:px-6 md:pt-14">
        <div><span className="mb-3 inline-block text-xs font-semibold tracking-wider text-primary-700">PAKISTAN&apos;S TRUSTED PROPERTY PORTAL</span><h1 className="mb-4 text-3xl font-bold leading-tight text-gray-900 md:text-5xl">Find a Property with Ease and <span className="text-primary-700">Confidence</span></h1><p className="mb-6 max-w-md text-base text-gray-500 md:text-lg">Explore thousands of verified properties for sale, rent and investment across Pakistan.</p><div className="flex gap-8">{stats.map((stat) => <div key={stat.label}><div className="text-2xl font-bold text-primary-700 md:text-3xl">{stat.value}</div><div className="text-sm text-gray-500">{stat.label}</div></div>)}</div></div>
...
function SearchBox() {
  ...
  return <div className="relative z-10 mx-auto -mt-6 max-w-6xl px-4 md:-mt-10 md:px-6"><div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-xl md:p-6">
    <div className="mb-5 flex gap-6 overflow-x-auto border-b border-gray-100 text-sm font-medium text-gray-500">{tabs.map((tab) => <button type="button" key={tab} onClick={() => selectTab(tab)} className={"whitespace-nowrap pb-3 " + (activeTab === tab ? "border-b-2 border-primary-700 text-primary-700" : "hover:text-gray-800")}>{tab}</button>)}</div>
    <form onSubmit={search} className="grid grid-cols-2 gap-3 md:grid-cols-5">
      ...
      <button type="submit" className="col-span-2 flex items-center justify-center gap-2 rounded-lg bg-primary-700 text-sm font-medium text-white transition hover:bg-primary-800 md:col-span-1"><Search className="h-4 w-4" /> Search Properties</button>
    </form>
    <div className="mt-5 flex flex-wrap items-center gap-2"><span className="mr-1 text-xs font-medium text-gray-500">Popular Searches:</span>{popularSearches.map((city) => <button type="button" key={city} onClick={() => chooseCity(city)} className="rounded-full border border-gray-200 px-3 py-1 text-xs font-medium text-gray-600 hover:border-primary-600 hover:text-primary-700">{city}</button>)}</div>
  </div></div>;
}
